...
class LaneDetection:
    ...
    def cut_gray(self, state_image_full):
        ...
        cut_image = cut_image[
            ..., :3
        ]  # select channel(R,G,B so it is 3) maintaining all dimension
    ...
    def find_maxima_gradient_rowwise(self, gradient_sum):
        ...
        for i in range(gradient_sum.shape[0]):  # iterate for row wise
            ...
            peak_idx = find_peaks(
                gradient_sum[i], distance=self.distance_maxima_gradient
            )[
                0
            ]  # first return value is index of peak
    ...
    def lane_detection(self, state_image_full):
        ...
        if lane_found:
            ...
            for row in range(1, self.cut_size):
                ...
                #    or if the distance to the next one is too big (>=100)
                if (
                    np.max(np.sqrt((row_max - lane1_prev[0][0]) ** 2)) >= 100
                    or np.max(np.sqrt((row_max - lane2_prev[0][0]) ** 2)) >= 100
                ):
                    break
                ...
                lane_boundary1_points = np.append(
                    lane_boundary1_points, lane1_prev, axis=0
                )
                lane_boundary2_points = np.append(
                    lane_boundary2_points, lane2_prev, axis=0
                )
            ...
            if (
                ...
                lane_boundary1, _ = splprep(
                    lane_boundary1_points.T, k=3, s=self.spline_smoothness
                )
                # lane_boundary 2
                lane_boundary2, _ = splprep(
                    lane_boundary2_points.T, k=3, s=self.spline_smoothness
                )
